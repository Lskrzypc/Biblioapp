...
const BooksPage: React.FC = () => {
  ...
  const handleAddBook = (newBook: Omit<BookModel, 'id'>) => {
    axios.post(`${process.env.NEXT_PUBLIC_API_URL}/books`, newBook)
      .then(response => {
        // Le backend renvoie l'objet livre complet après la création, y compris l'ID.
...
>>>>>>> 9bf92e03e703c02bcb19c185972ad2ea5baf3015
  ...
  return (
    <div className="flex flex-col gap-y-10">
      ...
      <div className="flex flex-wrap gap-x-10 gap-y-10 pl-6 pr-6">
        {books.map(book => (
          <div className="w-80 h-20 flex items-center border-2 border-gray-project rounded-lg" key={book.id}>
            <div className='pl-3 h-16 w-20 '><img className='object-cover h-full w-full rounded-lg' src={book.photoUrl} alt=''></img></div>
            <p className='font-outfit font-semibold text-gray-project pl-3 w-full text-left'>{book.title} - {book.author}</p>
            <button aria-label="View book" className='pr-5' onClick={() => handleBookClick(book.id)}>
      ...
      <span className='flex font-regular text-white items-center justify-center'>
