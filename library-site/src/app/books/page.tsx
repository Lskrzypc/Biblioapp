    ...
    <div className="flex flex-col gap-y-10">
      ...
      <div className="flex flex-wrap gap-x-10 gap-y-10 pl-6 pr-6">
        {filteredBooks.map((book) => (
          <div
            className="w-80 h-20 flex items-center border-2 border-gray-project rounded-lg"
            key={book.id}
          >
            ...
            <div className="flex flex-col justify-center pl-3">
              ...
              <div className="flex">
                {book.genres.map((genre, index) => (
                  <span
                    key={index}
                    className="text-xs font-light text-gray-project mr-2"
                  >
                    {genre}
                  </span>
                ))}
      ...
      <span className="flex text-xs font-regular text-white items-center justify-center">
        <button
          ...
          aria-label="Add book"
          className="px-8 h-10 bg-green-project rounded-xl"
        >
          Ajouter un livre
        </button>
      </span>

      <AddBookModal isOpen={isModalOpen} setIsModalOpen={setIsModalOpen} />
    </div>
  );
};
